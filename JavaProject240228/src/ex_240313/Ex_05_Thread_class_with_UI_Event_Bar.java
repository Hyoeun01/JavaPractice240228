...
class MyLabel extends JLabel {
	...
	// 생성자
	// 응용순서 4. 생성자 호출시 수치라벨의 인스턴스를 받아서 그 인스턴스를 계속 재사용
	...
	// 응용순서 5.consume이든 fill이든 둘다 repaint()를 호출함
	...
	public void paintComponent(Graphics g) {
		...
		// 분홍색 막대 그래프가 없을 때,
		  ...
		  // 응용순서 6. 수치를 나타내는 바의 라벨 > 
		//중요 ::  // 넘겨받은 sizeBarLabel의 인스턴스는 동일한 인스턴스를 재사용
		  // 문자열 라벨에 바의크기 붙이기 작업
...
//	SizeBarLabel sizeBarLabel = new SizeBarLabel();
	...
	// 응용순서 1 > 수치라벨을 붙이기위한 인스턴스 필요
	JLabel sizeBarLabel = new JLabel();
	
	// 응용순서 2 > 수치 라벨의 인스턴스를 mylabel에서 스레드로, 바크기에 맞게 그려지고있음.
	// sizebarlabel 인스턴스를 mylabel에 던져서 보냈어.
	// 왜? 스레드가 동작중인데, mylabel이 동작할때 매번 barsize의 크기가 지정이되니
	// mylabel의 크기를 > sizebarlabel : 크기값을 전달을했음
	// call by value는 메소드안에서 값이 사용후 휘발되지만,
	// ref를 사용하면 계속 그 인스턴스를 유지한다.
